...
test.describe('local (on-device) AI mode', () => {
  test('the "Local AI models" list is on Settings regardless of campaign or AI mode', async ({ page }) => {
    ...
    await expect(page.locator(DEFAULT_MODEL_ROW).getByRole('button', { name: 'Download' })).toBeVisible()
  ...
  test('is selectable in Settings, defaults to a specific model, and persists across a reload', async ({ page }) => {
    await installGoogleApiMock(page)
    await createRandomCampaign(page)
    ...
    await expect(page.locator('[data-slot="select-trigger"]').first()).toContainText('Local model (runs on this device)')
    // The campaign's own model picker (a second select) defaults to Gemma 3 1B and shows its size.
    await expect(page.locator('[data-slot="select-trigger"]').nth(1)).toContainText('Gemma 3 1B')
  })
  ...
  test('a model can be downloaded ahead of time from Settings, without acting first', async ({ page }) => {
    await installGoogleApiMock(page)
    await page.route(/huggingface\.co|hf\.co/, (route) => route.abort('failed'))
    ...
    const otherRow = page.locator('[data-testid="local-model-row-onnx-community/Qwen2.5-0.5B-Instruct"]')
    await expect(otherRow.getByRole('button', { name: 'Download' })).toBeEnabled()
  })
  ...
  test('a partially-downloaded model can be cleared even though it was never usable', async ({ page }) => {
    await installGoogleApiMock(page)
    await page.goto('/settings')
    ...
    await page.evaluate((modelId) => {
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        openReq.onsuccess = () => {
          ...
          const tx = db.transaction(['blocks', 'meta'], 'readwrite')
          tx.objectStore('blocks').put({ url, blockIndex: 0, bytes: new Uint8Array(8) })
          tx.objectStore('meta').put({ url, receivedBytes: 8, blockCount: 1, etag: null })
          tx.oncomplete = () => {
            db.close()
            resolve()
          }
    ...
    await expect(row.getByRole('button', { name: 'Download' })).toBeVisible()
    const clearButton = row.getByRole('button', { name: 'Clear partial download' })
