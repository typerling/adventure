 ...
 */
test('Kokoro is selectable as a text-to-speech provider in Settings', async ({ page }) => {
  ...
  const ttsTrigger = page.locator('[data-slot="select-trigger"]').nth(2)
  await ttsTrigger.click()

  const kokoroOption = page.getByRole('option', { name: 'Kokoro (on-device, runs locally)' })
...
test('the Kokoro voice model can be downloaded ahead of time from Settings', async ({ page }) => {
  ...
  await expect(page.getByText('Kokoro voice model', { exact: true })).toBeVisible()
  const downloadButton = page.getByRole('button', { name: 'Download voice model now' })
  await expect(downloadButton).toBeVisible()

  await downloadButton.click()
  // Goes into a disabled in-flight state rather than staying idle, so there's visible feedback.
  await expect(page.getByRole('button', { name: 'Download voice model now' })).toHaveCount(0)
...
test('an already-downloaded Kokoro voice model shows as ready and can be removed', async ({ page }) => {
  await installGoogleApiMock(page)
  await page.goto('/settings')
  ...
  await page.reload()
  await expect(page.getByText('Voice model downloaded and ready — playback starts instantly.')).toBeVisible()

  const removeButton = page.getByRole('button', { name: 'Remove downloaded voice model' })
  await expect(removeButton).toBeVisible()
  await removeButton.click()

  await expect(page.getByText('Voice model removed from this device.')).toBeVisible()
