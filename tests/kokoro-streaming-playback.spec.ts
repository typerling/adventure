 ...
 * the startup buffer, multi-voice pauses, WebGPU-restart de-duplication) — this spec exercises the
 * actual behavior change plus the guarantees that behavior change put at risk (stop() truly
 * silencing everything immediately).
 *
 ...
 * "start on chunk 1" design falling behind generation. Real (unfaked) Kokoro CPU inference (kokoro-
 * js's Node/onnxruntime-node backend — see kokoroTts.ts's "Startup playback buffer" doc comment)
 * found every generated chunk already has a clean silent taper at both ends — ruling out an in-model
 * boundary defect — and only a thin, uncertain generation-speed margin over real-time even on that
 * favorable backend, making "falling behind" the most plausible surviving explanation. The fix:
 ...
 * the pre-buffer streaming/stop() guarantees — real, unfaked Web Audio scheduling — plus (the third
 * test) that the buffer was re-derived against, not just ported onto, issue #66's later per-chunk
 * voice/pause model rather than assumed to still apply unchanged.
 */
...
test("a turn with fewer chunks than the startup buffer still plays once it's fully ready", async ({
  ...
  // pushing the chunk count up — the point here is proving flushPending's bufferTarget =
  // min(KOKORO_PLAYBACK_BUFFER_CHUNKS, chunks.length) logic doesn't wait forever for a second chunk
  // a one-chunk turn will never produce.
  await installGoogleApiMock(page);
...
test("a voice change spanning the startup buffer still gets its pause inserted (issue #66 interaction)", async ({
  page,
}) => {
  // Exercised directly against the provider with two single-sentence segments, each its own voice
  // (issue #66's multi-voice playback), so chunk 0 (segment A's one chunk) and chunk 1 (segment B's
  // one chunk) land on opposite sides of a voice change *while both are still inside the
  // KOKORO_PLAYBACK_BUFFER_CHUNKS=2 startup buffer*. This is the reconciliation this ticket actually
  // had to verify, not assume: pauseForVoiceChange's pause is computed at *schedule* time (inside
  // scheduleChunk, sequentially, from flushPending) rather than at arrival time — see kokoroTts.ts's
  // doc comment ("Reconciling this with multi-voice chunks") — which is what lets it apply correctly
  // even to two chunks flushed together in the very same synchronous batch, not just to chunks that
  // arrive one at a time after the buffer has already flushed once.
  await installGoogleApiMock(page);
  await installFakeKokoroModule(page, { chunkDurationSec: 0.5 });
  await installKokoroSourceTracking(page);
  ...
  await page.evaluate(async () => {
    ...
    mod
      ...
      .speak("", {
        segments: [
          { text: "The narrator sets the scene.", voice: "af_heart" },
          { text: "Hello there, the NPC says.", voice: "am_adam" },
        ],
      })
      .finally(() => {
        w.__kokoroTestSettled = true;
  ...
  // Chunk 0 (0.5s duration, nothing to pause after — the very first chunk never gets a pause before
  // it) starts at ctx.currentTime (~0); chunk 1 starts 0.5s (chunk 0's own duration) plus
  // KOKORO_ENTER_DIALOGUE_PAUSE_SEC (0.35s — no narratorVoice given here, so every voice change gets
  // the "entering dialogue" pause, see pauseForVoiceChange) later — even though both were scheduled
  // in the very same synchronous flushPending() batch, not one after the other across two separate
  // flushes.
  expect(starts[1].when - starts[0].when).toBeCloseTo(0.5 + 0.35, 2);
});
